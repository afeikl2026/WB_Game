...
const state = { activeCategory:'lobby', activeSub:'All', search:'', filters:{}, sort:'Popular', showLatestWins:false, currentView:'casino', activeProvider:null, providerSearch:'' };
...
function render(){ const isCasinoView=state.currentView==='casino'; const isCategory=isCasinoView&&state.activeCategory!=='lobby'; cancelLatestWinsShift(); renderNav(); discoveryControls.hidden=!isCasinoView; jackpot.hidden=!isCasinoView; discoveryControls.classList.toggle('is-category',isCategory); document.querySelector('#openFilter').classList.toggle('is-active',isCategory && Object.keys(state.filters).length > 0); document.querySelector('.toolbar-sort').classList.toggle('is-active',isCategory && state.sort !== 'Popular'); document.querySelector('#sortSelect').value=''; content.innerHTML=state.currentView==='providers'?renderProviderList():state.currentView==='providerGames'?renderProviderGames():isCategory?renderCategory():renderLobby(); observeShimmers(); observeGameCards(); }
...
function setupEvents(){
  ...
  content.addEventListener('click',e => { const game=e.target.closest('[data-game-id]'); if(game){const found=allGames.find(g=>g.id===game.dataset.gameId)||vipGames.find(g=>g.id===game.dataset.gameId);showToast(`Launching ${found?.name || 'game'} · demo mode`);return;} const provider=e.target.closest('[data-provider-id]');if(provider){state.activeProvider=provider.dataset.providerId;state.currentView='providerGames';render();window.scrollTo({top:0,behavior:'smooth'});return;} if(e.target.closest('#providerGamesBack')){state.currentView='providers';render();window.scrollTo({top:0,behavior:'smooth'});return;} if(e.target.closest('#providerBack')){state.currentView='casino';render();window.scrollTo({top:0,behavior:'smooth'});return;} if(e.target.closest('#viewLatestWins')){state.showLatestWins=true;render();window.scrollTo({top:0,behavior:'smooth'});return;} if(e.target.closest('#closeLatestWins')){state.showLatestWins=false;render();return;} const sub=e.target.closest('[data-sub]');if(sub){state.activeSub=sub.dataset.sub;render();return;} });
