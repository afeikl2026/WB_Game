...
const state = { activeCategory:'lobby', activeSub:null, lastLiveSub:null, search:'', filters:{}, sort:'Popular', currentView:'casino', lobbyDetailCategory:null, activeProvider:null, providerSearch:'', providerGameType:'All', recentProviderIds:loadRecentProviderIds(), activeGame:null, gameDetailReturn:null, gameDetailStack:[], detailFullscreen:false, detailCurrency:'USDT', detailDescriptionExpanded:false, gamePlayMode:null, gamePlayType:null, gamePlayPanelOpen:false };
...
function renderCategory(){ if(state.activeCategory==='live')activateLiveSubcategory(); const games = sorted(filtered(categoryGames())); const liveChips = state.activeCategory === 'live' ? `<div class="chip-row">${enabledLiveSubcategories().map(category => `<button class="sub-chip ${state.activeSub===category.id?'active':''}" data-sub="${category.id}" type="button">${category.name}</button>`).join('')}</div>` : ''; return `<section class="section category-page">${liveChips}${games.length ? `<div class="game-grid">${games.map(g => gameCard(g, state.activeCategory === 'live' ? 'live-card' : '')).join('')}</div>` : '<div class="empty-state">No games match these filters. Clear filters to restore the full category.</div>'}</section>`; }
...
function setupEvents(){
  ...
  content.addEventListener('click',e => {
    ...
    const sub=e.target.closest('[data-sub]');if(sub){activateLiveSubcategory(sub.dataset.sub);render();return;}
  });
